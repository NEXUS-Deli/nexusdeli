  ...
  useEffect(() => {
    if (selectedProduct && companyId) {
      TrackingService.trackEvent(companyId, {
        ...
        value: selectedProduct.promotional_price || selectedProduct.price,
      }).catch((err) => console.error("Track ViewContent err:", err));
    }
  ...
  useEffect(() => {
    if (showCheckout && companyId) {
      TrackingService.trackEvent(companyId, {
        ...
        value: cartTotal,
      }).catch((err) => console.error("Track InitiateCheckout err:", err));
    }
  }, [showCheckout, cartTotal, companyId]);
